...
app.post("/register" , async(req , res)=>{
    const{name , username , email , password , age} = req.body;  // Destructuring
    const user = await userModel.findOne({email});  // Here  we are checking that account is pre login or not with the email .
    ...
    bcrypt.genSalt(10 , (err , salt)=>{
        bcrypt.hash(password , salt , async(err , hash)=>{
            ...
            let token = jwt.sign("email" , "secretPasswordHere");
